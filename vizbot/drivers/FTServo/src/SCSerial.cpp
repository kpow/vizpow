/*
 * SCSerial.cpp — FTServo hardware interface layer (ESP-IDF UART)
...
{
	if (uart_num >= UART_NUM_MAX) return 0;
	...
	while (1) {
		...
		if (available > 0) {
			...
			if (len > 0) {
				if (nDat) nDat[size] = byte;
				size++;
			}
		}

		if (size >= nLen) break;
		if ((esp_timer_get_time() / 1000) - t_begin > static_cast<int64_t>(TimeOut)) break;
...
int SCSerial::readSCS(unsigned char *nDat, int nLen)
{
	...
	while (1) {
		...
		if (size >= nLen) break;
		if ((esp_timer_get_time() / 1000) - t_begin > static_cast<int64_t>(IOTimeOut)) break;
		vTaskDelay(pdMS_TO_TICKS(1));
	}
...
{
	if (uart_num >= UART_NUM_MAX || nDat == NULL) return 0;
