/*
 * SCSCL.cpp — SCSCL serial servo application layer
...
int SCSCL::SwitchMode(int ID, u8 mode)
{
	...
	if(mode == 1){
		...
		if(min_angle[servo_id] == -1 || max_angle[servo_id] == -1) return -3;
		angle_limit_cached[servo_id] = true;
		return PWMMode(servo_id);
	}

	if(!angle_limit_cached[servo_id]) return -4;
	if(writeWord(servo_id, SCSCL_MIN_ANGLE_LIMIT_L, (u16)min_angle[servo_id]) != 1) return -5;
	if(writeWord(servo_id, SCSCL_MAX_ANGLE_LIMIT_L, (u16)max_angle[servo_id]) != 1) return -6;
	return 0;
}
