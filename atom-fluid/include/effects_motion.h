...
void tiltBall() {
  ...
  // Axes match the Atom's calibrated fluid orientation: horizontal <- accelX
  // (left/right tilt), vertical <- accelY (up/down tilt).
  float targetX = 2.0 + accelX * 5.0 * ACCEL_SENSITIVITY;
  float targetY = 2.0 + accelY * 5.0 * ACCEL_SENSITIVITY;

  ballX += (targetX - ballX) * 0.5;  // Faster response (was 0.3)
  ballY += (targetY - ballY) * 0.5;
  
  ballX = constrain(ballX, 0, 4);
  ballY = constrain(ballY, 0, 4);
...
void tiltRipple() {
  ...
  float cx = 2.0 + accelX * 4.0 * ACCEL_SENSITIVITY;
  float cy = 2.0 - accelY * 4.0 * ACCEL_SENSITIVITY;
